...
def main():
    # 设置参数
    model_path = '../../runs/obb/train2/weights/best.kmodel'  # 替换为你的 kmodel 模型路径
    ...
    # 后处理
    detections = postprocess(predictions, scale, conf_threshold, iou_threshold)
    # 绘制检测结果
    result_image = draw_result(image.copy(), detections, class_names,colors)
    # 保存结果图像
    cv2.imwrite("kmodel_obb_result.jpg", result_image)
    print("kmodel_obb_result.jpg is saved!")
